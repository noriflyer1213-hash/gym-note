...
const saveData = (newMenus, newHistory) => {
...
rest: MASTER_EXERCISES.find(e => e.name === ex.name)?.rest ?? 120,
}));
const checked = {};
const overrides = {};
exs.forEach(ex => {
rec.exercises.find(e => e.name === ex.name)?.done?.forEach((d, i) => {
if (d) checked[`${ex.id}-${i}`] = true;
});
rec.exercises.find(e => e.name === ex.name)?.setData?.forEach((sd, i) => {
if (sd && (sd.weight !== ex.weight || sd.reps !== ex.reps)) {
overrides[`${ex.id}-${i}`] = { weight: sd.weight, reps: sd.reps };
}
});
});
...
done: Array.from({ length: ex.sets }, (*, i) => !!checkedSets[`${ex.id}-${i}`]),
setData: Array.from({ length: ex.sets }, (*, i) => getSetVal(ex, i)),
})),
};
...
saveData(undefined, historyRef.current.filter(r => r.id !== id));
setView(“home”);
};
...
saveData(undefined, historyRef.current.map(r => r.id === editingRecord.id ? editingRecord : r));
setSelectedHistory(editingRecord);
setView(“detail”);
};
...
<div style={S.scroll} onTouchMove={workoutSort.onTouchMove} onTouchEnd={workoutSort.onTouchEnd}>
...
setEditingRecord(p => ({
…p, exercises: p.exercises.map((x, j) => j !== ei ? x : {
…x, sets: newSets,
done: Array.from({ length: newSets }, (*, k) => x.done[k] ?? false),
setData: Array.from({ length: newSets }, (_, k) => x.setData?.[k] ?? { weight: x.weight, reps: x.reps }),
})
}));
}} />
